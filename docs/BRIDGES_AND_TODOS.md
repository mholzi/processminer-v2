...
* **The Problem (remaining):** the skill prompts still reference tools that have **no definition in `src/`**. Grouped by the right fix:
  * **Rewrite onto existing tools (no new tool needed):** `writeOverview` / `updateProcessOverview` → `updateElement` (already handles the overview, `doc.meta.id === id`); `getNextId` / `generateNextId` → drop (createElement assigns the id); `getElementTemplate` / `getTemplate` → refer to the schema template; `checkEvidence` → `checkConformance` (covers provenance). **(source-cx already rewritten; the rest pending.)**
  * **New root-field tools:** `writeIngestReport` / `clearConflicts` / `addSource` — **document-ingest**, **conflict-resolution** (write `doc.ingest`).
  * **New notes subsystem:** `createNote` / `resolveNotes` — **comment-review** (`/api/notes` exists in-app).
  * **New session-cursor API (biggest, riskiest):** `getSessionStatus` / `startSession` / `advanceSession` / `buildQueue` — **qer-session**, **foundational-run** (a real cursor over `reviewState`).
...
* **Root-level writes** — tools for the overview and the `ingest` report. The overview now collapses onto `updateElement` (it already patches `doc.content` when `doc.meta.id === id`) — a skill rewrite, not a new tool. The `ingest` report (`writeIngestReport`) + `clearConflicts` / `addSource` still need small root-field tools. `summaries` / `targetReview` already have `writeSummary` / `writeTargetReview`.
* **Session-cursor API** — `getSessionStatus` / `startSession` / `advanceSession` / `buildQueue` for qer-session + foundational-run (a real cursor over `reviewState`). The biggest and riskiest remaining group.
* Until the above land, treat qer-session, document-ingest (overview+ingest steps), conflict-resolution and comment-review as **not yet runnable end-to-end** against the real backend, and keep `/dogfood-run` out of CI. The `source-*` skills' core write+report path is now covered.
