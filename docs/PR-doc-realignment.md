...
| **#83** | Dispatcher integrity — enforce the A1 approval gate at element-create time (shared `buildElement`), centralize `applyLint` on `stripRuntimeState`, document the LIB-2 reload invariant (audit LIB-1/2) | `fix/create-approval-gate-lib1` → `main` | Code + tests | **Open** (`pending`) |
