...
| **#91** | `listProcesses` resilience — guard the per-file `JSON.parse` so one corrupt process file skips instead of breaking the whole portfolio; pure `parseProcessListing` helper + tests (audit LIB-6) | `fix/list-processes-resilience` → `main` | Code + tests | **Open** (`pending`) |
