...
| **#90** | Modal a11y (2/2) — ProcessSwitcher: focus-trap both dialogs (⌘K palette + confirm), gated so only the topmost traps; preserves layered Esc + arrow nav (audit UI-6b) | `fix/processswitcher-a11y` → `main` | Code (UI) | **Open** (`pending`) |
