...
- **Impact:** Minor each.
- **Effort:** S each · **Recommendation:** (a) Re-port CSS as-is. (b) Re-port adapted (add `onSelect` to `ElementHovercard`, thread `onRefClick`/`goToElement`, `cursor:pointer`). (c) Verify-then-decide — is the progress banner sufficient, or route through `handleSend`?
...
- **A1 — Approval gate is dead code (regression in the rewrite itself).** The legacy `set_approval.py` blocked `approved` while any heading was still `proposed`/`web`. On the baseline, `conformance.ts` still *defines* `UNCONFIRMED_SOURCES = {proposed, web}` with a comment claiming "these block approval," but it is **referenced nowhere** — `setApproval` doesn't gate. An element of pure AI-proposed content can be marked `approved`. **Recommendation:** wire `UNCONFIRMED_SOURCES` back into `setApproval`. Effort S. *(This is the live half of the same hallucination-countermeasure contract that R9 protects.)*
