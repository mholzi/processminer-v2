...
- ✅ **FIXED.** Runtime state (`reviewState`, `lint`, `findingDismissals`) now lives in a sibling **runtime store** (`src/lib/runtime-store.ts` → `data/runtime/<slug>.json`, gitignored). `getProcess` reads it; `applyLint` (MCP + Gemini) and `/api/findings` write it. `cob-003` migrated; `cob-003.json` now holds zero runtime keys. Verified: the welcome screen still shows the resume-run widget + lint findings, sourced from the runtime store. *(The full `ORCHESTRATOR-PLAN.md` doc + the read-only orchestrator consumer remain R10, optional.)*
- ✅ **Guardrail hardened (post-audit).** The original guardrail was a `delete doc.lint` in the two AI dispatchers only — it missed `reviewState`/`sources` and never ran on the in-app write path, so the pre-R9 `funds-release.json` still carried `reviewState`/`lint`/`sources`. Now [`stripRuntimeState`](src/lib/process-doc-hygiene.ts) (one canonical key list, tested) runs before **every** process-doc write in `wiki-write.ts`. `funds-release.json` was backfilled through it; **no process JSON carries runtime state**. *(The two dispatcher write paths still self-`delete doc.lint`; folding them onto `stripRuntimeState` is a tiny follow-up deferred with the LIB-1/LIB-2 dispatcher work.)*
...
- **A2 — Partial migration.** `cob-003` and `funds-release` are migrated to JSON (the `funds-release-dogfood*` files are dogfood/test artifacts, not curated processes). Some other processes (sepa-payment-processing, periodic-review, bank-guarantee-issuance, etc.) had their Markdown deleted with no JSON replacement. If that content matters, a migration backfill is its own work item. *(Affects R4 cross-process views, which look thin until backfilled.)*
