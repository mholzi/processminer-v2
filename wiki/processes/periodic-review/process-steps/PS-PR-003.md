...
provenance: {"Inputs":{"source":"document","evidence":"Owner. STP Decision Engine. Eligibility. Low or Medium risk and completeness ≥ 92 and no open screening hit and no event-based trigger and product mix unchanged."},"Outputs":{"source":"document","evidence":"If eligible, the engine: Refreshes the client's risk rating (re-running the rating model). Writes the new nextReviewDate to the client master. Posts the decision and the full evidence snapshot to the Audit Ledger. Notifies the RM (FYI) — no client contact."},"What happens":{"source":"document","evidence":"Owner. STP Decision Engine. Eligibility. Low or Medium risk and completeness ≥ 92 and no open screening hit and no event-based trigger and product mix unchanged. If ineligible, the case is routed to Reviewer Triage with the reason spelt out (e.g. 'ID document expires in < 90 days; address bounce-back twice in 6 months')."},"Why it matters":{"source":"document","evidence":"~62 % of Low- and Medium-risk reviews complete straight-through using authoritative data already held by the bank (no client contact)."}}
---
## What happens
