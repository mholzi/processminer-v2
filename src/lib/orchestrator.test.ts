// Tests for the v0.4 Orchestrator — run with:  npm run test:lint
...
// ---- Fixture helpers -----------------------------------------------------
// Build the minimum shape of a ProcessDoc the orchestrator needs. The view is
// a sentinel — buildOrchestratorState doesn't actually look at the view in
// v0.4 (it reads sidecars off the doc); the param is reserved for v0.5+ when
// the orchestrator might surface state.coverage etc. from the view.

const SENTINEL_VIEW = {} as ProcessView;
...
function doc(opts: {
  ...
  return {
    ...
    lint:
      findings.length > 0
        ? { generatedAt: "2026-01-01T00:00:00Z", slug: opts.slug, findings }
        : undefined,
    ingest:
    ...
    reviewState:
      opts.runResumable !== undefined && opts.runResumable !== null
        ...
        : undefined,
    view: undefined as never, // not read in v0.4 orchestrator
  };
}
...
test("buildOrchestratorState: clean process returns no actions and clean health", () => {
  ...
  const state = buildOrchestratorState(SENTINEL_VIEW, d);
...
test("buildOrchestratorState: resume-run sits between conflicts and lint", () => {
  // No conflicts, but a 100-item run with 99 left (weight 50 + 99 = 149).
  // Plus 30 lint (weight 150). Lint just edges out the run by one — but with
  // a smaller lint backlog, the run wins.
  const dRunBeatsLint = doc({
    slug: "a",
    openLintFindings: 10,
    runResumable: { cursor: 0, total: 100 },
  });
  const sA = buildOrchestratorState(SENTINEL_VIEW, dRunBeatsLint);
  ...
  const sB = buildOrchestratorState(SENTINEL_VIEW, dLintBeatsRun);
...
test("buildAttentionFeed: reasons preserve the legacy phrasing", () => {
  ...
  // before v0.4. Migration must not change them.
