...
6. **Conform to the schema.** Each element must follow its JSON schema definition exactly. If the SME's answer is too thin or too long for a block's word range constraint, that's a prompt for one more clarifying question, not a reason to pad or truncate.
...
- **C** (Consulted)
- **I** (Informed)
Build the RACI assignments with the SME — do not guess R/A/C/I. Write the RACI assignments as the `raci` list relation in the role element spec (e.g., `["PS-COB-001:R", "PS-COB-002:A"]`). Use `updateElement` to add/modify role elements.
...
**Phase 8 — Validation.** Before closing, do a gap-analysis pass over what you wrote:
- Look for steps with no owner or no rationale.
- Look for exceptions or pain points not linked to any step.
